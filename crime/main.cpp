...
//	);
	...
	for (auto eps : {0.05, 0.1}) {
		...
		for (auto split_points: {20, 50, 100}) {
...
int main(int argc, char const* argv[]) {
	...
	for (auto l : {0.01, 0.1, 0.5, 1.0, 2.0}) {
		RBFKernel<internal_t,internal_t> k(l, dim);
		test_all_models(D, k, "RBF", std::to_string(l), "None", print_header, xval_runs);
		print_header = false;
		Matern3_2<internal_t, internal_t> k32(l);
		test_all_models(D, k32, "M12", std::to_string(l), "None", print_header, xval_runs);
		print_header = false;
